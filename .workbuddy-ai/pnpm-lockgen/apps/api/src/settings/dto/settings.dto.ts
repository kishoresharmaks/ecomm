...
export class GstPlatformAddressDto {
  @ApiProperty({ example: "12 Marketplace Avenue" })
  @IsString()
  @MinLength(2)
  @MaxLength(180)
  line1!: string;
  ...
  @Matches(/^[0-9]{6}$/, { message: "postalCode must contain exactly 6 digits." })
  postalCode!: string;
...
export class GstPlatformIdentityDto {
  @ApiProperty({ example: "1HandIndia Private Limited" })
  @IsString()
  @MinLength(2)
  @MaxLength(160)
  legalName!: string;
  ...
  @Matches(/^[0-9]{2}$/, { message: "stateCode must contain exactly 2 digits." })
  stateCode!: string;

  @ApiProperty({ type: GstPlatformAddressDto })
  @ValidateNested()
...
export class GstManualWorkflowDto {
  ...
  @ApiProperty({ enum: ["MANUAL"], example: "MANUAL" })
  @IsIn(["MANUAL"])
  provider!: "MANUAL";
}

export class GstEWayBillWorkflowDto extends GstManualWorkflowDto {
...
export class UpsertGstSettingsDto {
  @ApiProperty({ type: GstPlatformIdentityDto })
  @ValidateNested()
  @Type(() => GstPlatformIdentityDto)
  platform!: GstPlatformIdentityDto;

  @ApiProperty({ type: GstManualWorkflowDto })
  @ValidateNested()
  @Type(() => GstManualWorkflowDto)
  eInvoice!: GstManualWorkflowDto;

  @ApiProperty({ type: GstEWayBillWorkflowDto })
  @ValidateNested()
  @Type(() => GstEWayBillWorkflowDto)
  eWayBill!: GstEWayBillWorkflowDto;
}
