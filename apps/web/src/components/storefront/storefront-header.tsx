...
export function StorefrontHeader({ initialMenu }: { initialMenu?: CmsMenuItem[] | undefined }) {
  ...
  const mobileCategoryLinks = primaryCategories.slice(0, 4);
  const mobileCmsItems = cmsItems
  ...
  return (
    <header className="sticky top-0 z-[80] border-b border-[#F1D7CF] bg-white/95 shadow-[0_10px_32px_rgba(22,59,92,0.04)] backdrop-blur-xl">
      <div className="mx-auto max-w-md px-3 pb-3 pt-2 lg:hidden">
        <div className="flex items-center justify-between gap-2">
          <button
            type="button"
            onClick={() => setMobileOpen((current) => !current)}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-[#111827] transition hover:bg-[#FFF0EC] hover:text-[#ED3500]"
          ...
          <div className="flex shrink-0 items-center justify-end gap-1">
            <Link href="/account/wishlist" className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-[#111827] transition hover:bg-[#FFF0EC] hover:text-[#ED3500]" aria-label="Wishlist">
              <Heart className="h-5 w-5" aria-hidden="true" />
            </Link>
            <Link href="/cart" className="relative grid h-9 w-9 shrink-0 place-items-center rounded-full text-[#111827] transition hover:bg-[#FFF0EC] hover:text-[#ED3500]" aria-label="Cart">
              <ShoppingCart className="h-5 w-5" aria-hidden="true" />
              {totals.itemCount > 0 ? (
                <span className="absolute right-0 top-0 grid h-4 min-w-4 place-items-center rounded-full bg-[#ED3500] px-1 text-[9px] font-black text-white">
                  {totals.itemCount}
                </span>
              ) : null}
            </Link>
        ...
        <form onSubmit={submitSearch} className="mt-3">
          <label className="relative block">
            ...
            <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#ED3500]" />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search products, stores, brands..."
              className="h-12 w-full rounded-full border border-[#FFB9A6] bg-white pl-11 pr-[94px] text-[12px] font-semibold text-[#111827] shadow-[0_12px_28px_rgba(22,59,92,0.06)] outline-none transition focus:border-[#ED3500] focus:ring-4 focus:ring-[#ED3500]/10"
            />
            <button
              type="submit"
              className="absolute right-1.5 top-1.5 h-9 rounded-full bg-[#ED3500] px-4 text-[11px] font-black text-white shadow-[0_10px_20px_rgba(237,53,0,0.20)] transition hover:bg-[#d52f00]"
            >
              Search
            </button>
          </label>
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            ...
            className="inline-flex h-11 shrink-0 items-center gap-2 rounded-[12px] bg-[#ED3500] px-3 text-[12px] font-black text-white shadow-[0_12px_24px_rgba(237,53,0,0.18)] transition hover:bg-[#d52f00]"
          >
          ...
          <button
            type="button"
            onClick={() => setMobileOpen(true)}
            className="ml-auto inline-flex h-10 items-center gap-1.5 rounded-full px-3 text-[12px] font-black text-[#111827] transition hover:bg-[#FFF0EC] hover:text-[#ED3500]"
          >
        ...
        {mobileOpen ? (
          <>
            <div className="absolute left-0 right-0 top-full z-[85] lg:hidden">
              <div className="mx-auto max-w-md px-3 pt-2">
                <div className="indihub-mobile-drawer max-h-[min(560px,62dvh)] overflow-hidden rounded-[20px] border border-[#FFE0D6] bg-[#FFFCFB] shadow-[0_20px_54px_rgba(22,59,92,0.14)]">
                  <div className="max-h-[inherit] overflow-y-auto overscroll-contain p-3 [-webkit-overflow-scrolling:touch]">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <MobileLink href="/categories" label="All Categories" icon={<Grid3X3 className="h-4 w-4" />} onNavigate={() => setMobileOpen(false)} className="col-span-2" />
                      {mobileCategoryLinks.map((category, index) => {
                    ...
                    <div className="mt-3 border-t border-[#FFE0D6] pt-3">
                      <AuthActions />
                    </div>
                  </div>
                </div>
...
function MobileBrandLogo() {
  return (
    <Link href="/" className="flex min-w-0 flex-1 items-center gap-1.5" aria-label="1HandIndia home">
      <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-white shadow-[0_10px_20px_rgba(237,53,0,0.14)]">
        <img src={brandLogoSrc} alt="" className="h-full w-full object-cover" loading="eager" />
      </span>
      <span className="min-w-0">
        <span className="block max-w-[118px] truncate text-[18px] font-black leading-none tracking-normal text-[#111827] min-[390px]:max-w-[146px]">
          Hand<span className="text-[#ED3500]">India</span>
        </span>
        <span className="mt-0.5 block max-w-[118px] truncate text-[8px] font-semibold leading-none text-[#667085] min-[390px]:max-w-[146px]">
...
function MobileLink({
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={cn(
        "flex min-w-0 items-center gap-2 rounded-[14px] border border-[#FFE0D6] bg-white px-2.5 py-2.5 text-[12px] font-black text-[#1F2933] transition hover:-translate-y-0.5 hover:border-[#ED3500]/40 hover:bg-[#FFF7F3]",
        className,
      )}
    >
      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#FFF0EC] text-[#ED3500]">{icon}</span>
