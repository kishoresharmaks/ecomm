export const adminNav = [
  { group: "Overview", label: "Dashboard", href: "/admin" },
  { group: "People", label: "Customers", href: "/admin/customers" },
  { group: "People", label: "Business buyers", href: "/admin/business-buyers" },
  { group: "People", label: "Users & roles", href: "/admin/users" },
  { group: "People", label: "Sellers", href: "/admin/sellers" },
  { group: "People", label: "Seller approvals", href: "/admin/sellers/approvals" },
  { group: "People", label: "Delivery partners / riders", href: "/admin/delivery-partners" },
  { group: "Commerce", label: "Products", href: "/admin/products" },
  { group: "Commerce", label: "Product approvals", href: "/admin/products/approvals" },
  { group: "Commerce", label: "Categories", href: "/admin/categories" },
  { group: "Commerce", label: "Services", href: "/admin/services" },
  { group: "Commerce", label: "Service bookings", href: "/admin/service-bookings" },
  { group: "Commerce", label: "Deals", href: "/admin/deals" },
  { group: "Commerce", label: "Coupons", href: "/admin/coupons" },
  { group: "Commerce", label: "Orders", href: "/admin/orders" },
  { group: "Commerce", label: "Returns", href: "/admin/returns" },
  { group: "Commerce", label: "Reviews", href: "/admin/reviews" },
  { group: "Commerce", label: "B2B enquiries", href: "/admin/b2b-enquiries" },
  { group: "Commerce", label: "B2B orders", href: "/admin/b2b-orders" },
  { group: "Commerce", label: "B2B analytics", href: "/admin/b2b/analytics" },
  { group: "Commerce", label: "Support", href: "/admin/support" },
  { group: "Commerce", label: "Chat support", href: "/admin/chat" },
  { group: "Content", label: "Content management", href: "/admin/cms" },
  { group: "Locations", label: "Locations", href: "/admin/locations" },
  { group: "Locations", label: "Location import", href: "/admin/locations/import" },
  { group: "Locations", label: "Location serviceability", href: "/admin/locations/serviceability" },
  { group: "Operations", label: "Delivery operations", href: "/admin/delivery" },
  { group: "Operations", label: "Delivery applications", href: "/admin/delivery-partner-applications" },
  { group: "Operations", label: "Courier workspace", href: "/courier" },
  { group: "Operations", label: "Storage", href: "/admin/storage" },
  { group: "Operations", label: "Search index", href: "/admin/search" },
  { group: "Operations", label: "Email", href: "/admin/email" },
  { group: "Operations", label: "Push campaigns", href: "/admin/push-campaigns" },
  { group: "Operations", label: "Audit logs", href: "/admin/audit-logs" },
  { group: "Operations", label: "Settings", href: "/admin/settings/general" },
  { group: "Operations", label: "Reports", href: "/admin/reports" },
  { group: "Finance", label: "Payments", href: "/admin/payments" },
  { group: "Finance", label: "B2B payment verification", href: "/admin/b2b-payments" },
  { group: "Finance", label: "Refunds", href: "/admin/refunds" },
  { group: "Finance", label: "Seller subscriptions", href: "/admin/seller-subscriptions" },
  { group: "Finance", label: "Commission rules", href: "/admin/finance/commission-rules" },
  { group: "Finance", label: "Settlements", href: "/admin/finance/settlements" },
  { group: "Finance", label: "Payouts", href: "/admin/finance/payouts" },
  { group: "Finance", label: "Seller ledger", href: "/admin/finance/ledger" },
  { group: "Finance", label: "Statements", href: "/admin/finance/statements" },
  { group: "Finance", label: "Finance workspace", href: "/finance" },
];

export const financeNav = [
  { group: "Overview", label: "Dashboard", href: "/finance" },
  { group: "Collections", label: "COD collections", href: "/finance/cod-collections" },
  { group: "Collections", label: "Bank transfers", href: "/finance/bank-transfers" },
  { group: "Collections", label: "B2B payment proofs", href: "/admin/b2b-payments" },
  { group: "Collections", label: "Payment status", href: "/finance/payment-status" },
  { group: "Seller finance", label: "Settlements", href: "/finance/settlements" },
  { group: "Seller finance", label: "Payouts", href: "/finance/payouts" },
  { group: "Seller finance", label: "Ledger", href: "/finance/ledger" },
  { group: "Seller finance", label: "Statements", href: "/finance/statements" },
  { group: "Rules", label: "Commission rules", href: "/finance/commission-rules" },
  { group: "Reports", label: "Payment reports", href: "/finance/reports" },
  { group: "Settings", label: "Payment settings", href: "/finance/settings" },
];

export const courierNav = [
  { group: "Overview", label: "Dashboard", href: "/courier" },
  { group: "Packages", label: "Packages", href: "/courier/packages" },
  { group: "Packages", label: "Routing failures", href: "/courier/routing-failures" },
  { group: "Local delivery", label: "Local delivery", href: "/courier/local-delivery" },
  { group: "Local delivery", label: "Delivery partners", href: "/courier/local-delivery/partners" },
  { group: "Courier setup", label: "Providers", href: "/courier/providers" },
  { group: "COD", label: "COD remittances", href: "/courier/cod-remittances" },
];

export const sellerNav = [
  { label: "Dashboard", href: "/seller", capability: "ALL" },
  { label: "Store profile", href: "/seller/store-profile", capability: "ALL" },
  { label: "Onboarding", href: "/seller/register", capability: "ALL" },
  { label: "Products", href: "/seller/products", capability: "RETAIL" },
  { label: "Services", href: "/seller/services", capability: "SERVICE" },
  { label: "Service bookings", href: "/seller/service-bookings", capability: "SERVICE" },
  { label: "Service calendar", href: "/seller/service-calendar", capability: "SERVICE" },
  { label: "Deals", href: "/seller/deals", capability: "RETAIL" },
  { label: "Coupons", href: "/seller/coupons", capability: "RETAIL" },
  { label: "Orders", href: "/seller/orders", capability: "RETAIL" },
  { label: "Returns", href: "/seller/returns", capability: "RETAIL" },
  { label: "Reviews", href: "/seller/reviews", capability: "RETAIL" },
  { label: "B2B Enquiries", href: "/seller/b2b-enquiries", capability: "RETAIL" },
  { label: "B2B Orders", href: "/seller/b2b-orders", capability: "RETAIL" },
  { label: "Reports", href: "/seller/reports/sales", capability: "ALL" },
  { label: "Subscription", href: "/seller/subscription", capability: "ALL" },
  { label: "Wallet", href: "/seller/finance/wallet", capability: "ALL" },
  { label: "Payouts", href: "/seller/finance/payouts", capability: "ALL" },
  { label: "Statements", href: "/seller/finance/statements", capability: "ALL" },
] as const;
