...
export const serverEnvSchema = z.object({
  ...
  PG_APP_NAME: z.string().optional(),
  PG_POOL_MAX: z.coerce.number().int().positive().optional(),
  PG_POOL_CONNECTION_TIMEOUT_MS: z.coerce.number().int().positive().optional(),
  PG_POOL_IDLE_TIMEOUT_MS: z.coerce.number().int().positive().optional(),
  PG_POOL_MAX_LIFETIME_SECONDS: z.coerce.number().int().positive().optional(),
  ...
  API_CORS_ORIGINS: z.string().default("http://192.168.1.2:3000"),
  ...
  FX_PROVIDER: z.enum(["frankfurter", "currencyapi"]).default("frankfurter"),
