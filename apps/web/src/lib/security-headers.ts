...
export function buildContentSecurityPolicy({ nonce, origin }: SecurityHeaderOptions) {
  ...
  const directives = [
    ...
    ["style-src", "'self'", "'unsafe-inline'", `'nonce-${nonce}'`, "https://fonts.googleapis.com", ...analyticsOrigins, ...clerkOrigins, ...parseCsvOrigins(process.env.NEXT_PUBLIC_CSP_STYLE_SRC)],
    ["style-src-elem", "'self'", "'unsafe-inline'", "https://fonts.googleapis.com", ...analyticsOrigins, ...clerkOrigins, ...parseCsvOrigins(process.env.NEXT_PUBLIC_CSP_STYLE_SRC)],
    ["style-src-attr", "'unsafe-inline'"],
    ["img-src", "'self'", "https:", "data:", "blob:", apiOrigin, ...parseCsvOrigins(process.env.NEXT_PUBLIC_CSP_IMG_SRC)],
    ["font-src", "'self'", "data:", "https://fonts.gstatic.com", ...analyticsOrigins, ...clerkOrigins, ...parseCsvOrigins(process.env.NEXT_PUBLIC_CSP_FONT_SRC)],
    [
      "connect-src",
      "'self'",
      apiOrigin,
    ...
    ["frame-ancestors", "'self'", ...parseCsvOrigins(process.env.NEXT_PUBLIC_CSP_FRAME_ANCESTORS)],
