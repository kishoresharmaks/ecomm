...
-- Includes the approved baseline through 20260721203000_service_sac_tax_integration,
...
CREATE OR REPLACE FUNCTION "prevent_b2b_verified_payment_core_update"() RETURNS TRIGGER AS $$ BEGIN   IF OLD."status" IN ('VERIFIED', 'CLEARED') AND (     NEW."b2b_order_id" IS DISTINCT FROM OLD."b2b_order_id" OR     NEW."requested_schedule_id" IS DISTINCT FROM OLD."requested_schedule_id" OR     NEW."method" IS DISTINCT FROM OLD."method" OR     NEW."amount_paise" IS DISTINCT FROM OLD."amount_paise" OR     NEW."unallocated_amount_paise" IS DISTINCT FROM OLD."unallocated_amount_paise" OR     NEW."currency" IS DISTINCT FROM OLD."currency" OR     NEW."reference_number" IS DISTINCT FROM OLD."reference_number" OR     NEW."proof_file_key" IS DISTINCT FROM OLD."proof_file_key" OR     NEW."provider_order_id" IS DISTINCT FROM OLD."provider_order_id" OR     NEW."provider_payment_id" IS DISTINCT FROM OLD."provider_payment_id" OR     NEW."provider_order_creation_in_progress" IS DISTINCT FROM OLD."provider_order_creation_in_progress" OR     NEW."provider_order_created_at" IS DISTINCT FROM OLD."provider_order_created_at" OR     NEW."provider_method" IS DISTINCT FROM OLD."provider_method" OR     NEW."provider_payload" IS DISTINCT FROM OLD."provider_payload" OR     NEW."cheque_number" IS DISTINCT FROM OLD."cheque_number"   ) THEN     RAISE EXCEPTION 'Verified B2B payment financial fields are immutable.';   END IF;   RETURN NEW; END; $$ LANGUAGE plpgsql;  CREATE TRIGGER "b2b_payment_records_verified_immutable" BEFORE UPDATE ON "b2b_payment_records" FOR EACH ROW EXECUTE FUNCTION "prevent_b2b_verified_payment_core_update"();  CREATE OR REPLACE FUNCTION "prevent_b2b_receipt_voucher_change"() RETURNS TRIGGER AS $$ BEGIN   IF TG_OP = 'UPDATE' AND      OLD."file_key" IS NULL AND      NEW."file_key" IS NOT NULL AND      ROW(NEW."id", NEW."voucher_number", NEW."payment_record_id", NEW."issued_by_id", NEW."issued_at", NEW."created_at")        IS NOT DISTINCT FROM      ROW(OLD."id", OLD."voucher_number", OLD."payment_record_id", OLD."issued_by_id", OLD."issued_at", OLD."created_at") THEN     RETURN NEW;   END IF;   RAISE EXCEPTION 'Issued B2B receipt vouchers are immutable.'; END; $$ LANGUAGE plpgsql;  CREATE TRIGGER "b2b_receipt_vouchers_immutable" BEFORE UPDATE OR DELETE ON "b2b_receipt_vouchers" FOR EACH ROW EXECUTE FUNCTION "prevent_b2b_receipt_voucher_change"();  CREATE OR REPLACE FUNCTION "prevent_b2b_immutable_pod_change"() RETURNS TRIGGER AS $$ BEGIN   RAISE EXCEPTION 'Issued B2B POD evidence is immutable.'; END; $$ LANGUAGE plpgsql;  CREATE TRIGGER "b2b_proofs_of_delivery_immutable" BEFORE UPDATE OR DELETE ON "b2b_proofs_of_delivery" FOR EACH ROW EXECUTE FUNCTION "prevent_b2b_immutable_pod_change"();  CREATE OR REPLACE FUNCTION "prevent_closed_b2b_qc_change"() RETURNS TRIGGER AS $$ BEGIN   IF OLD."closed_at" IS NOT NULL THEN     RAISE EXCEPTION 'Closed B2B QC inspections are immutable.';   END IF;   RETURN NEW; END; $$ LANGUAGE plpgsql;  CREATE TRIGGER "b2b_qc_inspections_closed_immutable" BEFORE UPDATE OR DELETE ON "b2b_qc_inspections" FOR EACH ROW EXECUTE FUNCTION "prevent_closed_b2b_qc_change"();
