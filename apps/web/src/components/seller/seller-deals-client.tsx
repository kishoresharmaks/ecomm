    ...
    <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
      ...
      <section className="rounded-lg border border-[#D8E2EA] bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3">
          {dealsQuery.error ? <SellerErrorPanel error={dealsQuery.error} onRetry={() => void dealsQuery.refetch()} /> : null}
          ...
          {!dealsQuery.isLoading && !dealsQuery.error && !dealsQuery.data?.items.length ? (
            <SellerEmptyState title="No deal campaigns" message="No published deal campaigns are available right now." />
          ) : null}
        </div>
      </section>

      <section className="rounded-lg border border-[#D8E2EA] bg-white p-5 shadow-sm">
        {detailQuery.error ? <SellerErrorPanel error={detailQuery.error} onRetry={() => void detailQuery.refetch()} /> : null}
        {detail ? (
          <>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      confirmation.requestConfirmation({
                        ...
                        description: `"${detail.title}" will be marked as declined and its eligible products will not be enrolled.`,
                        confirmLabel: "Decline deal",
                        onConfirm: () => declineMutation.mutate(detail.id),
                      })
                    }
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
                {(detail.eligibleProducts ?? []).map((product) => (
                  <SellerDealProductCard
                    ...
                    onRemove={() =>
                      confirmation.requestConfirmation({
                        title: "Remove product from this deal?",
                        description: `"${product.name}" will stop participating in "${detail.title}".`,
                        confirmLabel: "Remove product",
                        onConfirm: () => removeMutation.mutate({ dealId: detail.id, productId: product.id }),
                      })
                    }
                  />
                ))}
        ...
        ) : !dealsQuery.error && !detailQuery.error ? (
    ...
    <article className="rounded-md border border-[#E5E7EB] bg-[#FCFDFE] p-3">
      <div className="flex gap-3">
        <Link href={`/products/${product.slug}` as Route} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-md bg-white">
          <StorefrontImage src={primaryImage(product)} alt={product.name} sizes="80px" fallbackLabel={product.category?.name ?? product.name} />
        </Link>
